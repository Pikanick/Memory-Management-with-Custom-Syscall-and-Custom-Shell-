...
void addNodes0ToN(struct nodeStruct **headRef, int n, _Bool atHead);
...
void testAddManyToHeadOrTail(_Bool addToHead) {
  ...
  TEST(List_findNode(list, 0)->item == 0);
  TEST(List_findNode(list, 1)->item == 1);
  TEST(List_findNode(list, 2)->item == 2);
  TEST(List_findNode(list, 3)->item == 3);
  TEST(List_findNode(list, 4)->item == 4);
  TEST(List_findNode(list, 5)->item == 5);
}
